...
public class searchDoctors extends JDialog
{
    ...
    public searchDoctors(JFrame parentFrame, JButton throwSearchButton, JButton searchMainButton, TableRowSorter<AbstractTableModel> rowSorter, doctorsTableModel dtm)
    {
        super(parentFrame, "Поиск", true);

        Vector data = new Vector();
        data.addAll(dtm.getColumnRUSNames());
        data.add("Все столбцы");
        columnNames = new JComboBox(data);
        columnNames.setFocusable(false);
        columnNames.setSelectedIndex(10);
            ...
            if(searchTextField.getText().trim().equals(""))
            {
                ...
                switch (columnNames.getSelectedIndex())
                {
                    ...
                    case 9:
                        ...
                        break;
                    case 10:
                        rowSorter.setRowFilter(new myRowFilter(searchTextField.getText().trim(), 10).regexFilter("(?iu)" + searchTextField.getText().trim())); // "(?iu)" - игнор регистра
                        break;
                }
            }
        });
